...
- (void)showImage
{
    ...
    BOOL cunzai = [manager diskImageExistsForURL:_photo.url];
    if (cunzai) //本地缓存
    {
        _photo.isDownLoad = YES;
//        if (_photo.firstShow) {
//            _photo.isDownLoad = NO;
//        }
...
- (void)adjustFrame
{
    ...
    // 内容尺寸
    self.contentSize = CGSizeMake(boundsWidth, boundsHeight);
    ...
    if (!_photo.isDownLoad || _photo.firstShow) {
        
        _imageView.frame = [_photo.srcImageView convertRect:_photo.srcImageView.bounds toView:nil];
        
        if (!_photo.firstShow || !_photo.isDownLoad) { // 第一次显示的图片
            _imageView.center = CGPointMake(self.width/2, self.height/2);
        }
        
        _photo.firstShow = NO; // 已经显示过了
        _photo.isDownLoad = YES; //标记为已下载
...
//    }
    ...
    [UIView animateWithDuration:duration + 0.1 animations:^{
        ...
        if ((!rect.origin.x&&!rect.size.width) || (rect.origin.y>self.height)) {
            _imageView.alpha = 0;
...
- (void)handleDoubleTap:(UITapGestureRecognizer *)tap {
    ...
    float zoom = self.maximumZoomScale/self.zoomScale;
    
    CGPoint touchPoint = [tap locationInView:self];
	if (self.zoomScale == self.maximumZoomScale) {
		[self setZoomScale:self.minimumZoomScale animated:YES];
	} else {
        [self zoomToRect:CGRectMake(touchPoint.x*zoom, touchPoint.y*zoom, 1, 1) animated:YES];
