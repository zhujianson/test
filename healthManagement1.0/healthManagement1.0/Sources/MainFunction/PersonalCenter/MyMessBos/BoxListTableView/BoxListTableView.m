...
- (void)viewDidLoad
{
    ...
    boxListTableView.separatorColor = [CommonImage colorWithHexString:LINE_COLOR];
...
//    [dic setObject:@"-1" forKey:@"acquisitionTime"];
...
- (void)getLocalData
{
    ...
    if (!m_isGetDataForServer) {
        ...
        NSString *time = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"%@boxLastRowTime%d", g_nowUserInfo.userid,self.m_mail]];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        ...
        UILabel * m_labSubTitle = [[UILabel alloc] initWithFrame:CGRectMake(m_read_type.right+7, m_labTitle.bottom+6, m_labTitle.width, 16)];
        m_labSubTitle.backgroundColor = [UIColor clearColor];
        m_labSubTitle.tag = 1232;
        m_labSubTitle.textColor = [CommonImage colorWithHexString:@"999999"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    int red = [[oneDic objectForKey:@"isRead"] boolValue];
    if (!red)
    {
        NSString *sql = [NSString stringWithFormat:@"UPDATE systemMsg%d SET isRead = '%d' WHERE broadcastId = '%@'", self.m_mail,1, oneDic[@"broadcastId"]];
//        oneDic[@"broadcastId"],
        [[DBOperate shareInstance] insertLocalDataForSQL:sql];
        [oneDic setObject:@"1" forKey:@"isRead"];
        [boxListTableView reloadData];
        
        g_nowUserInfo.broadcastNotReadNum = MAX(g_nowUserInfo.broadcastNotReadNum-red, 0);
    }
    ...
    if (![[head objectForKey:@"state"] intValue])
    {
        ...
        if ([loader.username isEqualToString:URL_getBroadcastList]) {
            if ([resultArray count]) {
                [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%@", resultArray[0][@"createTime"]] forKey:[NSString stringWithFormat:@"%@boxLastRowTime%d", g_nowUserInfo.userid,self.m_mail]];
