...
@property(nonatomic,strong) ALButton *closeButton;
@end
...
- (id)initWithMoviePlayer:(ALMoviePlayerController *)moviePlayer style:(ALMoviePlayerControlsStyle)style {
    ...
    if (self) {
        ...
        _barColor = [[UIColor grayColor] colorWithAlphaComponent:0.5];
        ...
        _barHeight = [UIDevice iOSVersion] >= 7.0 ? 44.f : 44.f;//高度
...
- (void)setup {
    ...
    _durationSlider.minimumTrackTintColor = [CommonImage colorWithHexString:VERSION_TEXT_COLOR];
    ...
    if (_style == ALMoviePlayerControlsStyleFullscreen || (_style == ALMoviePlayerControlsStyleDefault && _moviePlayer.isFullscreen)) {
        ...
        [_fullscreenButton setTitle:@"返回" forState:UIControlStateNormal];
...
//        [_bottomBar addSubview:_volumeView];
        ...
        if (!_closeButton)
        {
            ...
            [_closeButton setImage:[UIImage imageNamed:@"common.bundle/common/closeMovie.png"] forState:UIControlStateNormal];
            [_closeButton addTarget:self action:@selector(closeTheMoview) forControlEvents:UIControlEventTouchUpInside];
...
- (void)movieFinished:(NSNotification *)note {
    ...
    if ([self.moviePlayer.delegate respondsToSelector:@selector(movieFinishedCallback:)])
    {
        NSLog(@"++++%@",self.moviePlayer.delegate);
        if (!self.moviePlayer.fullscreen)
        {
             [(KXMoviePlayer *)self.moviePlayer.delegate  movieFinishedCallback:nil];
        }
        else
        {
...
//        self.fullscreenButton.frame = CGRectMake(self.fullscreenButton.right+paddingBetweenButtons , self.barHeight/2 - playHeight/2, playWidth, playHeight);
//        self.timeElapsedLabel.frame = CGRectMake(self.fullscreenButton.frame.origin.x + self.fullscreenButton.frame.size.width + paddingBetweenButtons, 0, labelWidth, self.barHeight);
        self.timeElapsedLabel.frame = CGRectMake( self.playPauseButton.right + paddingBetweenButtons, 0, labelWidth, self.barHeight);
...
//        }
        ...
        self.airplayView.frame = self.fullscreenButton.frame;
        ...
        self.closeButton.frame = CGRectMake(self.bounds.size.width-22-5, 5, 22, 22);
    }
