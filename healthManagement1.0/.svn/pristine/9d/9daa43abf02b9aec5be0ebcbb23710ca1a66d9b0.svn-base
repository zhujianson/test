...
- (NSData*)CompactAudio
{
    NSData* data = EncodeWAVEToAMR([NSData dataWithContentsOfFile:self.recordPath], 1, 16);
    
    [[NSFileManager defaultManager] removeItemAtPath:self.recordPath error:nil];
    
    NSString *rootFilePath = [Common getAudioPath];
    
    NSString *amrFilePath = [rootFilePath stringByAppendingFormat:@"/%@", self.m_audioName];
    [data writeToFile:amrFilePath atomically:YES];
    
    return data;
}

- (void)startRecorderCompletion:(XHStartRecorderCompletion)startRecorderCompletion {
    _isPause = NO;
    NSError *error = nil;
    NSLog(@"1");
...
//    [recordSetting setValue:[NSNumber numberWithInt:kAudioFormatAppleIMA4] forKey:AVFormatIDKey];
    ...
    // File URL
    NSString *rootFilePath = [Common getAudioPath];
    self.m_audioName = nil;
    self.m_audioName = [NSString stringWithFormat:@"%@_%ld_%d", g_nowUserInfo.userid, [CommonDate getLongTime], arc4random()%1000];

    self.recordPath = [rootFilePath stringByAppendingFormat:@"/%@",self.m_audioName];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (_peakPowerForChannel) {
                _peakPowerForChannel(peakPowerForChannel, self.currentTimeInterval);
            }
        });
