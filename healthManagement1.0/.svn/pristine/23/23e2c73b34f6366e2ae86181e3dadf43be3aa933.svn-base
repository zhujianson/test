...
//REGULAREXPRESSION(PhoneNumerRegularExpression, @"^[0-9]*$")
...
const CGFloat kEmojiOriginYOffsetRatioWithLineHeight = 0.15; //表情绘制的y坐标矫正值，和字体高度的比例，越大越往下
...
//    [self setValue:[NSArray array] forKey:@"links"];
    ...
    UIColor *commonLinkColor = [CommonImage colorWithHexString:@"576b95"];
    
    //点击时候的背景色
    [mutableActiveLinkAttributes setValue:(__bridge id)[[CommonImage colorWithHexString:@"c2c2c4"] CGColor] forKey:(NSString *)kTTTBackgroundFillColorAttributeName];
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            if (imageName) {
                ...
                UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"common.bundle/msg/emo/small/%@.png", imageName]];
...
- (void)setEmojiText:(NSString*)emojiText
{
    ...
    if ([_emojiDelegate respondsToSelector:@selector(customHandleNameString)])
    {
         NSMutableArray *nameArray = [_emojiDelegate customHandleNameString];
        [results addObjectsFromArray:nameArray];
    }
...
- (void)attributedLabel:(TTTAttributedLabel *)label didSelectLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
{
    ...
    if(self.emojiDelegate&&[self.emojiDelegate respondsToSelector:@selector(customDidSelectLink:)]){
        //type的数组和i刚好对应
        [self.emojiDelegate customDidSelectLink:result.replacementString];
    }
}
...
{
    NSString *returnString = emojiText;
    if (emojiText.length)
    {
        NSRange deleteRange = [MLEmojiLabel emojiTextisContainsEmojiWithText:emojiText];
        NSString *newString = [MLEmojiLabel deleteEmojiTextWithOriginalString:emojiText withEmojiRange:deleteRange];
...
+ (NSRange)emojiTextisContainsEmojiWithText:(NSString *)emojiText
{
    NSArray *emojis = nil;
    NSString *returnString = emojiText;
    NSRange range = NSMakeRange(returnString.length-1, 1);
    if (emojiText.length)
    {
        ...
        emojis = [customEmojiRegularExpression matchesInString:emojiText
                                                       options:NSMatchingWithTransparentBounds
                                                         range:NSMakeRange(0, [emojiText length])];
        NSTextCheckingResult *result = [emojis lastObject];
        NSRange resultRange = result.range;
//        NSMutableString *newString = nil;
//        newString = [[NSMutableString alloc]initWithString:emojiText];
        //最后一个是表情
        if ((resultRange.location + resultRange.length) == emojiText.length &&([[self class] judgeFaceStringWithFaceString:[returnString substringWithRange:resultRange]]))
        {
            range = resultRange;
//            [newString deleteCharactersInRange:range];
//            returnString = [NSString stringWithFormat:@"%@",newString];
        }
        else
        {
            //系统表情
            NSRange mEmojiRange = NSMakeRange(returnString.length-2, 2);
            NSString *mEmojiString = [returnString substringWithRange:mEmojiRange];//是否为对应的表情
            if ([NSString isContainsEmoji:mEmojiString])
            {
               range = mEmojiRange;
            }
        }
...
+(NSString *)deleteEmojiTextWithOriginalString:(NSString *)emojiText withEmojiRange:(NSRange )mEmojiRange
{
    NSMutableString *newString = nil;
    newString = [[NSMutableString alloc] initWithString:emojiText];
    [newString deleteCharactersInRange:mEmojiRange];
    return newString;
}
...
///
///  @return 存在是yes
+(BOOL) judgeFaceStringWithFaceString:(NSString *)faceString
{
    NSString *plistStr = [[NSBundle mainBundle] pathForResource:@"expression" ofType:@"plist"];
    NSDictionary *m_plistDic = [[NSDictionary alloc] initWithContentsOfFile:plistStr];
    if ([[m_plistDic allKeys] containsObject:faceString])
    {
        return YES;
    }
    return NO;
...
-(void)UIMenuControllerWillHideMenuNotification
{
    if (_isCopyIng)
    {
        //取消copy
        if ([self.copyableLabelDelegate respondsToSelector:@selector(stringToCopyForCopyableLabel:withChangeColor:)])
        {
              [self.copyableLabelDelegate stringToCopyForCopyableLabel:nil withChangeColor:YES];
        }
        _isCopyIng = NO;
    }
}
...
- (void)copy:(id)sender
{
    if (self.copyingEnabled)
    {
        ...
        if ([self.copyableLabelDelegate respondsToSelector:@selector(stringToCopyForCopyableLabel:withChangeColor:)])
        {
            stringToCopy = [self.copyableLabelDelegate stringToCopyForCopyableLabel:self withChangeColor:YES];
        }
        else
        {
